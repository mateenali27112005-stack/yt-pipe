...
try {
  ...
  const next = await generateAndPublishVisualAsset(spec, manifest, assetId, { outputDirectory: assetDirectory, manifestPath: manifestFile, provider: getImageProvider(providerName) });
...
function usage(): never { throw new Error("Usage: npm run visual-generate -- <shot_visual_spec.json> <asset_manifest.json> <asset-id> --provider <fake|openai> --overwrite-manifest"); }
