...
function makeValidAIFF(numSampleFrames = 44100): Buffer {
  ...
  for (let i = 0; i < 100; i++) ssndData.writeInt16BE(1000, 8 + (i * 2));
...
class FakeRenderer implements Renderer {
  async render(context: FinalCompositionSpec, options: { outputPath: string }): Promise<RenderResult> {
    await writeFile(options.outputPath, Buffer.from("fake mp4 data"));
